...
(defn between
  ...
  [start last]
  #?(:clj (filter (fn [{date :date}]
                    (and (>= (.compareTo date start) 0)
                         (<= (.compareTo date last) 0)))
                  (vals holiday-jp.holidays/holidays))
     :clje (filter (fn [{date :date}]
                     (and (>= (.timestamp date) (.timestamp start))
                          (<= (.timestamp date) (.timestamp last))))
                   (vals holiday-jp.holidays/holidays))
     :cljr (reverse (filter (fn [{date :date}]
                              (and (>= (.CompareTo date start) 0)
                                   (<= (.CompareTo date last) 0)))
                            (vals holiday-jp.holidays/holidays)))
     :cljs (filter (fn [{date :date}]
                     (and (>= (.getTime date) (.getTime start))
